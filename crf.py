...
class CRF(nn.Module):
    def __init__(self, feat_num, num_tags):
        super(CRF, self).__init__()
        
        self.num_tags = num_tags + 2 # add start and end
        self.start_idx = self.num_tags - 2
        self.stop_idx = self.num_tags - 1
        
        self.fc = nn.Linear(feat_num, self.num_tags) # \phi(y_i, x_i)
        self.transitions = nn.Parameter(torch.randn(self.num_tags, self.num_tags), requires_grad=True) # \phi(t_i, t_i+1)
    ...
    def __viterbi_decode(self, features, masks):
        ...
        for t in range(L):
            ...
            # print(max_score.size(), acc_score_t.size(), mask_t.size())
